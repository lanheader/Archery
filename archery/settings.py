...
if ENABLE_LDAP_DATA_COMPLETION or ENABLE_LDAP:
    ...
    AUTH_LDAP_USER_SEARCH_BASE = env("AUTH_LDAP_USER_SEARCH_BASE", default="xxxx")
    ...
    AUTH_LDAP_USER_ATTR_MAP_STRING = env(
        "AUTH_LDAP_USER_ATTR",
        default={"username": "uid", "display": "uid", "email": "mail"},
    )
    AUTH_LDAP_USER_ATTR_MAP = {
        x.split("=")[0]: x.split("=")[1]
        for x in AUTH_LDAP_USER_ATTR_MAP_STRING.split(",")
    }
if ENABLE_LDAP:
    AUTHENTICATION_BACKENDS = (
...
if ENABLE_CAS:
    ...
    AUTHENTICATION_BACKENDS = (
        "django.contrib.auth.backends.ModelBackend",
        "cas.auth.CASAuthenticationBackend",
    )
    # CAS 的地址
